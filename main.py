...
logging.basicConfig(filename='suplog.log', filemode='a', format='%(asctime)s - %(name)s - %(message)s',
                    level=logging.DEBUG)
...
class SupremeMonitor:
    ...
    def get_item_variants(self, item_id, item_name, start):
        ...
        for stylename in item_variants["styles"]:
            for itemsize in stylename["sizes"]:
                item = [item_name, stylename["name"], itemsize['name'], item_variants["description"],
                        'https:' + stylename["image_url"], item_url.split('.json')[0]]
    ...
    def discord_webhook(self, product_item):
        ...
        if product_item == 'initial':
            embed["description"] = "Monitor is online..."
        ...
        embed["footer"] = {'text': 'Powered by CoproyaleIO'}
    ...
    def monitor(self):
        ...
        initial = ['', '', '', "CoproyaleIO Supreme Monitor" '', '', '', ]
